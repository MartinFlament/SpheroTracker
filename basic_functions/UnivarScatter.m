...
%
%                       +'WhiskerWidthRatio' - Lines/boxes that represent
%                       the mean,SEM,std will be Width/WhiskerWidthRatio
...
%     data_istable=false;
% end
%comment because no table in 2012
 data_istable=false;
%% String Variables
if ~data_istable
...
Ind=strmatch('WhiskerWidthRatio',stringVars);
if ~isempty(Ind)
    %Check that WhiskerWidthRatio is a number
    if isnumeric(valueVars{Ind}) && all(size(valueVars{Ind})== [1,1])
        WhiskerWidthRatio=valueVars{Ind};
    else
        error(sprintf('wrong WhiskerWidthRatio, it should be a number'))
    end
else
   WhiskerWidthRatio=4;
end
...
if strcmp(Whiskers,'box')
    ...
    rectangle('Position',[i-Width/WhiskerWidthRatio,yMean-yStd,2*Width/WhiskerWidthRatio,2*yStd ],'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
    %plot the mean+-SEM box
    rectangle('Position',[i-Width/WhiskerWidthRatio,yMean-yCI,2*Width/WhiskerWidthRatio,2*yCI ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
    %plot the mean line 
    plot([i-Width/WhiskerWidthRatio i+Width/WhiskerWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex,:),'LineWidth',8)
...
%range_val=niginv([0.05 0.95], 1, 0,mean(yValues),std(yValues));
...
if strcmp(Whiskers,'lines')
    %plot the mean line
    yMean=nanmean(yValues);
    plot([i-Width/WhiskerWidthRatio i+Width/WhiskerWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex),'LineWidth',1.5)
    
    %plot the sd 
    yStd=std(yValues);
    plot([i-Width/WhiskerWidthRatio*0.5 i+Width/WhiskerWidthRatio*0.5],[yMean+yStd yMean+yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([i-Width/WhiskerWidthRatio*0.5 i+Width/WhiskerWidthRatio*0.5],[yMean-yStd yMean-yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([i i],[yMean-yStd yMean+yStd],'Color',StdColor(StdIndex,:))
    %plot the conf. interval of the mean
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;
    plot([i-Width/WhiskerWidthRatio*0.7 i+Width/WhiskerWidthRatio*0.7],[yMean+yCI yMean+yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
    plot([i-Width/WhiskerWidthRatio*0.7 i+Width/WhiskerWidthRatio*0.7],[yMean-yCI yMean-yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
