...
PathName='D:\data1\martinf\Films\Yeranuhi\Ca\MFM\';
SaveFolder='D:\data1\martinf\results\Yeranuhi\Ca\Analysis\MFM\';

dirs = dir(PathName);

for d=4:length(dirs)
  
    files = dir([PathName dirs(d).name '\Data_Ca\']);
    %files=dir([SaveFolder '\*f_PSCa.mat']);
    F = length(files);
    PixelSize = 1; %Indicate here the pixel size in micrometer !
    SamplingFrequency =1/0.0132; % Indicate here the framerate (in fps) !
        SaveName={};
    
    %%
    for f=1:F %Bug MFM 28-29-32-33-316
        ...
        %load([SaveFolder,FileName])
            ...
            PSCa = PeriodicSignalsCaModel(FileName,mask,[PathName dirs(d).name '\Data_Ca\'],'PixelSize',1,'SamplingFrequency',SamplingFrequency);
            ...
            PSCa.getinfo;
            info=imfinfo([PathName, dirs(d).name, '\Data_Ca\', FileName]);
...
list_param = {'File_Name', 'Period','Period_std', 'Amplitude', 'Ascend_time', 'Decay_time', 'Ascend_slope', 'Decay_slope'};%'Manip', 'Strain', 'Batch', 'Spheroid',
for S=3:length(samples)
    ...
    writetable(Param_Ca,[PathName 'Param_Ca.xlsx'], 'Sheet',samples(S).name)  ;
end
