...
%% all files
dossier = 'D:\data1\martinf\Films\Gabriel\SF Manip moins chouette (manip n1)\SF endo MM\';
dossier = 'D:\data1\martinf\Films\Yeranuhi\BF\CTL\';
dossier = uigetdir;


dirs = dirbranchendsh(dossier);

savedir = ['D:\data1\martinf\results\Gabriel\test\'];
savedir = ['D:\data1\martinf\results\Yeranuhi\BF\Analysis\TestGUIController\'];
savedir = uigetdir;
savedir = [savedir '\'];
...
g=GUImask_working(dirs{dd},savedir,param, 0);
waitfor(g); clear g
end
...
if param.stack
    for dd=1:length(dirs)
        ...
        g = GUIparam_working(dirs{dd}, 'SaveDir', savedir, 'Parameters', param,'num',1); %paramf(d)
              %param=paramf(d);
         waitfor(g)
...
% for ii=1:length(paramf)
% paramf(ii).auto=1;
% end
disp('run KLT:');
if param.stack
    for dd=51:length(dirs)
        list = listTiffs(dirs{dd});
        for d = 1:length(list)
             disp([num2str(d) ' | ' num2str(length(list))]);
             FileName = list{d};
...
list_param = {'File_Name', 'Num_Particles','Contractile_Particles', 'Period','Period_std', 'Amplitude', 'Amplitude_decay','Amplitude_std','Contraction_time', 'Relaxation_time', 'Contraction_slope', 'Relaxation_slope','Relaxation_slope_00_50', 'Relaxation_slope_50_100', 'Resting_time', 'Homogeneity', 'AUC_median', 'SigNoise'};
